<template>
  <q-header class="q-py-sm bg-white text-primary">
    <q-toolbar>
  ...
  <q-page v-else ref="dishList">
    <q-list v-if="dishes.length > 0">
      ...
      <q-expansion-item expand-icon-toggle switch-toggle-side v-for="dish in filteredDishes" :key="dish.dish.id"
        ...
        <q-item>
          <q-item-section>
            <div>
              <span class="text-bold">Zutaten pro Portion:</span> {{ dish.dish.ingredients.map(ingredient =>
                ingredientToString(ingredient)).join(', ') }}
            </div>
            <div v-if="dish.dish.subdishes && dish.dish.subdishes.length > 0" class="q-mt-md">
              <span class="text-bold">Sub-dishes:</span> {{ dish.dish.subdishes.map(subdish => `${dishes.find(dish =>
    ...
    <q-page-sticky position="bottom" :offset="[0, -28]" style="z-index: 6000;">
      <q-btn fab icon="checklist" color="accent" @click="onClickChooseIngredients"
        :disabled="disableChooseIngredients" />
    </q-page-sticky>

    <ChooseIngredientsDialog v-model:model-value="chooseIngredientsDialog.show" :dishes="chooseIngredientsDialog.dishes"
      @add="onClickAdd" />
